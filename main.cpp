...
typedef boost::multiprecision::number<
        boost::multiprecision::cpp_bin_float<100>> real_t;
...
int main(int, char **) {
    std::cout << std::setprecision(
            std::numeric_limits<real_t>::max_digits10 - 1);

    std::vector<zsvm::Particle<real_t>> particles;
    const std::string mass_carrier("mass");
    const std::string charge_carrier("charge");

    std::map<std::string, real_t> electron_carriers;
    electron_carriers.insert({mass_carrier, 1});
    electron_carriers.insert({charge_carrier, -1});

    std::map<std::string, real_t> positron_carriers;
...
//    std::cout << std::endl;
    ...
    std::vector<std::vector<real_t>> input_basis;
    for (const auto &line : basis_input_items) {
...
//    std::cout << std::endl;
    ...
    std::vector<real_t> best_basis_element;
    std::vector<std::vector<real_t>> thread_basis_elements(num_threads);
    std::vector<real_t> thread_energies(num_threads);
...
#pragma omp parallel
    {
        ...
        zsvm::SphericalECGJacobiVariationalOptimizer<real_t> optimizer(
        ...
        for (std::size_t basis_size = optimizer.get_basis().size();
            ...
            bool found = optimizer.expand_amoeba(
                    trials_per_thread, 1, max_iterations);
            if (found) {
